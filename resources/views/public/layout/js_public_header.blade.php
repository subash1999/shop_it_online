...
{{-- script for the cart and wishlist --}}
<script>
	{{-- for the cart and wishlist --}}
	function getCartTotalNumberOfItems() {
		$.ajax({
			url: '{{ url('cart/total_items_in_cart') }}',
			type: 'GET',	
			data : null,
	...
	function getCartTotalPrice() {
		$.ajax({
			url: '{{ url('cart/total_price_of_cart_with_currency_symbol') }}',
			type: 'GET',	
			data : null,				
		})
		.done(function(result) {
	...
	function getWishlistTotalNumberOfItems() {
		$.ajax({
			url: '{{ url('wishlist/total_items_in_wishlist') }}',
			type: 'GET',	
			data : null,
	...
	function getWalletAmountWithCurrencySymbol() {
		$.ajax({
			url: '{{ url('wallet/wallet_amount_with_currency_symbol') }}',
			type: 'GET',	
			data : null,
...
{{-- script for the currency change --}}
<script type="text/javascript">
	$(document).ready(function() {
		function selectedCurrency() {
			$.ajax({
				url: '{{ url('currency/selected_currency') }}',
				type: 'POST',	
				data : null,
		...
		function currencyOptions() {
			$.ajax({
				url: '{{ url('currency/currency_options') }}',
				type: 'POST',	
				data : null,
			...
			.done(function(options) {
				...
				for (var i = 0; i < options.length; i++) {
					var href_url = "{{ url('currency/select_currency') }}" +"/"+ options[i]["curr_id"];
...
{{-- script for the main category list in the search bar --}}
<script type="text/javascript">
	...
	// $(document).ready(function() {
		function getMainSearchCategory() {
			$.ajax({
				url: '{{ url('product/search/main_category_list') }}',
				type: 'GET',
			})
			.done(function(category_list) {
				...
				if(category_list!=null){
					...
					for (var i = 0; i < category_list.length; i++) {
						html +=  `<li onClick="clickMainCategoryOpen(${category_list[i]["cate_id"]})"><a href='{{ url('all_products/?category%5B%5D=') }}${category_list[i]["cate_id"]}' >${category_list[i]["cate_name"]}<i class="fas fa-chevron-right ml-auto"></i></a></li>`;
