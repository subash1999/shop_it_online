	...
	bootbox.confirm({
		...
		callback : function(result){
			if(result == true){
				...
				jQuery.ajax({
					url : "{{URL::to('admin/users/user_types/restore_deleted')}}",
			// {{-- url : "{{ URL::to('admin/users/user_types/'+ut_id)}}", --}}
				// url : 'admin/users/user_types/'.ut_id,
				method: 'POST',
				data: {
		...
		bootbox.confirm({
			...
			callback : function (result) {
				if(result==true){
					...
					jQuery.ajax({
						url : "{{url('admin/users/user_types')}}",
						// {{-- url : "{{ URL::to('admin/users/user_types/'+ut_id)}}", --}}
						// url : 'admin/users/user_types/'.ut_id,
						method: 'POST',
						data: {
						...
						success : function (result) {
							...
							+" <label style='font-size:20px;weight:bold;'>"+
							"{{ config('app.name', 'Shop IT Online') }}"+"</label> ";
							bootbox.alert({
								message: success_message,
								title: success_title,
								buttons: {
...
jQuery(document).ready(function($) {
	var table = $('#user_types_data_table').DataTable({
		...
		"processing" : true,
		"serverSide" : true,
		"ajax" : "{{ URL::to('admin/users/user_types/get_table') }}",
		"columns" : [
		{"data" : "ut_id"},
		{ "data" : "type"},
