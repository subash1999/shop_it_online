...
<div id="option_groups_collection">
	<div class="option_group" id="option_group_{{$option_group}}" name="option_group[{{$option_group}}]">
		<h4 class="h4">Option Group {{$option_group}} <button type="button" class="btn btn-theme04 float-right" onclick="deleteOptionGroup('option_group_{{$option_group}}')">Delete Option Group</button></h4>
		<div class="form-inline">
			<label class="control-label">Option Group Name<label style="font-size: x-small;"> (Max 255 character)</label></label>
			<input type="text" name="option_group[{{$option_group}}][option_group_name]" id="option_group_{{$option_group}}" class=" form-control m-3" placeholder="i.e. color,size,color and size,etc" maxlength="255">
		</div>
		<div class="from-group">
			<label class="control-label"><u>Option Available or the Option Group</u></label>
			<button type="button" class="btn btn-primary m-4" id="add_option_btn" name="add_option_btn" onclick="addOption('option_group_{{$option_group}}',{{$option_group}})">Add Option</button>
			<table class="table table-bordered option_group_table" id="option_group_table_{{$option_group}}">
				<thead>
					<th>Option</th>
					<th>Product Photo</th>
				...
				<tbody>					
					<tr class="option_row" id="option_row_{{$option}}">
						<td><input class="option form-control" type="text" name="option_group[{{$option_group}}][{{$option}}][option_name]" id="option_group_{{$option_group}}_option_{{$option}}" maxlength="255">								
						</td>
						<td>
							<div class="col-md-4">
								<div class="fileupload fileupload-new" data-provides="fileupload">
									<div class="fileupload-new thumbnail" style="width: 150px; height: 100px;">
										<img src="http://www.placehold.it/200x150/EFEFEF/AAAAAA&amp;text=no+image" alt="" name="option_group_{{$option_group}}_option_{{$option}}_img" id="option_group_{{$option_group}}_option_{{$option}}_img">
									</div>
									...
									<div>
										<span class="btn btn-theme02 btn-file">
											...
											<input type="file" class="default" accept="image/*" name="option_group[{{$option_group}}][{{$option}}][option_image]" class="option_group_{{$option_group}}_option_{{$option}}_input">
										</span>
						...
						<td>
							<input class="option form-control" type="number" name="option_group[{{$option_group}}][{{$option}}][number_of_items]" id="option_group_{{$option_group}}_option_{{$option}}_qty" min="0">
						</td>
						<td>
							<button type="button" class="btn btn-danger" id="delete_option_btn" name="delete_option_btn" onclick="deleteOptionRow('option_row_{{$option}}')">Delete Option</button>
						</td>
					</tr>
				</tbody>
			</table>
...
<script>
	var option_group_count = {{++$option_group}};
	var option_count = {{++$option}};
