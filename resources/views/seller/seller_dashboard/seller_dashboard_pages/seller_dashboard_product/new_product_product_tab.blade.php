...
<table class="product_form_table table"  >	
	<tbody>
		<tr>
			...
			<td >
				<input type="text" name="product_name" id="product_name" class="form-control" style="margin-left: 5px;" maxlength="255"  value="{{old('product_name')}}" required> 
		...
		<td>
			<div>			
				<select name="curr_id" id="curr_id" class="custom-select form-control-lg mb-3" required>
					...
					<option value="{{$currency->curr_id}}">{{$currency->currency_name}} ({{$currency->country}})</option>
					@endforeach
				</select>
			</div>
			...
			<div class="col-md-4">
				<div class="fileupload fileupload-new" data-provides="fileupload">
					<div class="fileupload-new thumbnail" style="width: 150px; height: 100px;">
						<img src="http://www.placehold.it/200x150/EFEFEF/AAAAAA&amp;text=no+image" alt="" name="product_image[{{$i}}]" id="product_image_{{$i}}">
					</div>
					...
					<div>
						<span class="btn btn-theme02 btn-file">
							...
							<span class="fileupload-exists"><i class="fa fa-undo"></i> Change</span>
							<input type="file" class="default" accept="image/*" name="product_image_input[{{$i}}]" id="product_image_input_{{$i}}">
						</span>
						<a href="javascript:void(0)" class="btn btn-theme04 fileupload-exists" data-dismiss="fileupload" onclick="deleteProductImage('product_image_{{$i}}','product_image_input_{{$i}}')"><i class="fa fa-trash-o"></i> Remove</a>
					</div>
				</div>
			</div>	
			@endfor	
